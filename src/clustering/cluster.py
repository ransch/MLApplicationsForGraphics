...
def main():
    ...
    kmeans = KMeans(n_clusters=hyperparams.clusteringCnt, init='k-means++', n_init=10, max_iter=300, tol=1e-4,
                    precompute_distances=True, algorithm='elkan').fit(lowDimMat)
    buckets = createBuckets(hyperparams.clusteringCnt, kmeans.labels_)
